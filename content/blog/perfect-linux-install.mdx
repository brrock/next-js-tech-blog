...
# arch, you have to xorg/wayland and a login manager for this method and let's be real if you know 
sudo pacman -S gnome
```

You can find similar snippets for other Desktop environment ( which I will call DE from now on ). Moving on to theming the default look for your chosen DE might not fit your tastes and themes for KDE, XFCE and gnome are easy to install. The easiest cross DE to theme is use lxapperance and have GTK themes downloaded and icon themes, if you use pling use OCS url to put them to the correct folder snd then use lxapperance to choose it. Now moving on to GRUB theming as I don’t have the space to write all about them, [here](https://christitus.com/bootloader-themes/) is a link to ChrisTitus’s great guide plus install script for all distros. Something that you do with grub is a dual boot so in my set up I boot of my external SSD (just a 500GB Samsung T7) and get a grub menu where I can choose between windows and Debian. I hope you enjoyed this guide and hope you have used this to make your perfect system.